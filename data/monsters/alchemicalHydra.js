const alchemicalHydra = {
	drops: {
		...
		dragonWeaponry: ['<:Dragon_knife:545975448446828584>', '<:Dragon_thrownaxe:403018313187328010>']
	},
	nameMap: {
	...
	smallKill(quantity) {
		...
		for (let i = 0; i < quantity; i++) {
			...
			if (this.roll(180)) {
				loot.push(this.drops.ringPieces[Math.floor(Math.random() * this.drops.ringPieces.length)]);
			}
			if (this.roll(2000)) {
				loot.push(this.drops.dragonWeaponry[Math.floor(Math.random() * this.drops.dragonWeaponry.length)]);
			}
		}
		return loot.join(' ');
	},
